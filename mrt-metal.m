...
static void init(void) {
    /* setup sokol_gfx with context from sokol_app */
    sg_setup(&(sg_desc){
        // .context = sapp_metal_get_device()
    ...
    state.offscreen_pass_desc = (sg_pass_desc){
        .color_attachments[0].image = sg_make_image(&color_img_desc),
        .color_attachments[1].image = sg_make_image(&color_img_desc),
        .color_attachments[2].image = sg_make_image(&color_img_desc),
    ...
    state.offscreen_pass_action = (sg_pass_action){
        .colors[0] = { .action = SG_ACTION_CLEAR, .value = { 0.25f, 0.0f, 0.0f, 1.0f } },
        .colors[1] = { .action = SG_ACTION_CLEAR, .value = { 0.0f, 0.25f, 0.0f, 1.0f } },
        .colors[2] = { .action = SG_ACTION_CLEAR, .value = { 0.0f, 0.0f, 0.25f, 1.0f } }
    };
    ...
    state.offscreen_pip = sg_make_pipeline(&(sg_pipeline_desc){
        ...
        .layout.attrs[0] = { .offset = offsetof(vertex_t,x), .format = SG_VERTEXFORMAT_FLOAT3 },
        .layout.attrs[1] = { .offset = offsetof(vertex_t,b), .format = SG_VERTEXFORMAT_FLOAT },
    ...
    state.fsq_bind.fs_images[0] = state.offscreen_pass_desc.color_attachments[0].image;
    state.fsq_bind.fs_images[1] = state.offscreen_pass_desc.color_attachments[1].image;
    state.fsq_bind.fs_images[2] = state.offscreen_pass_desc.color_attachments[2].image;
    ...
    state.fsq_pip = sg_make_pipeline(&(sg_pipeline_desc){
        ...
        .shader = sg_make_shader(&(sg_shader_desc){
            ...
            .fs.images[1] = { .image_type = SG_IMAGETYPE_2D },
            .fs.images[2] = { .image_type = SG_IMAGETYPE_2D },
            .fs.source =
                "#include <metal_stdlib>\n"
                "using namespace metal;\n"
                "struct fs_in {\n"
...
static void frame(void) {
    ...
    _sg.pass_valid = true;
    _sg_begin_pass(0, &state.default_pass_action, sapp_width(), sapp_height());
    // sg_begin_default_pass(&state.default_pass_action, sapp_width(), sapp_height());
    ...
    sg_apply_viewport(0 * 100, 0, 100, 100, false);
    state.dbg_bind.fs_images[0] = state.offscreen_pass_desc.color_attachments[0].image;
    sg_apply_bindings(&state.dbg_bind);
    sg_draw(0, 4, 1);

    sg_apply_viewport(1 * 100, 0, 100, 100, false);
    state.dbg_bind.fs_images[0] = state.offscreen_pass_desc.color_attachments[1].image;
    sg_apply_bindings(&state.dbg_bind);
    sg_draw(0, 4, 1);

    sg_apply_viewport(2 * 100, 0, 100, 100, false);
    state.dbg_bind.fs_images[0] = state.offscreen_pass_desc.color_attachments[2].image;
    sg_apply_bindings(&state.dbg_bind);
    sg_draw(0, 4, 1);
